...
def ModeloSequencial_Grande2(casename,casename2):
    
    global debug
    debug = False
    dataNv = loadmat(casename2)
  #  val_u = {key : data['Solution_Ship_var_u'][key][0][0][0][0] for key in data['Solution_Ship_var_u'].dtype.names}
    val_w = {key : dataNv['Solution_Ship_var_w'][key][0][0][0][0] for key in dataNv['Solution_Ship_var_w'].dtype.names}
    data = loadmat(casename)
    C = data['C'][0][0]
    R = data['R'][0][0]
    Patios = data['Patios'].tolist()
    Npatios = len(Patios) # numero de patios
 ...
 #   TT =data['TT']
   ...
   # model.parameters.timelimit.set(432000)  # limite de tempo em segundos (5 dias)
    ...
    Y = 'Instancia: %s <br> Rodado em: %s <br> Solution status: %s  <br> Valor da Funcao Objetivo de Carregamento: %s  <br> Duracao: %s <br>' %(casename,socket.gethostname(),status,fobj,solvetime)
                    ...
                    for oo in range(o):
                        for d in range(o+1,P):
                            for a in range(o+1,d+1):
                                rhs_current =+ -val_w['w_'+str(oo+1)+'_'+str(d+1)+'_'+str(a+1)+'_'+str(r+1)+'_'+str(c+1)]

                    rhs.append(float(rhs_current))            
          ...
          for r in range(R):
              for c in range(C):
                  for d in range(o+1,P):
                      ...
                      rhs_current = 0    
                      for a in range(o+1,d+1): 
                          rhs_current =+ val_w['w_'+str(o+1)+'_'+str(d+1)+'_'+str(a+1)+'_'+str(r+1)+'_'+str(c+1)]
                      rhs.append(float(rhs_current))
    ...
    for o in range(len(N)):
        for d in range(o+1,P):
            ...
            for oo in range(o):
                for r in range(R):
                    for c in range(C): 
                        rhs_current =+ val_w['w_'+str(oo+1)+'_'+str(d+1)+'_'+str(o+1)+'_'+str(r+1)+'_'+str(c+1)]
            rhs.append(float(rhs_current))            
            for r in range(R):
                for c in range(C):
                    cols.append(qind[(o+1,d+1,r+1,c+1)])
...
if __name__ == "__main__":
    ...
    for i in X:
        ...
        name_instance2 = 'Solution_Ship_InstanciaModeloIntegrado_' + str(i) + '_1.mat'
        print(name_instance1)
        _ = ModeloSequencial_Grande2(name_instance1,name_instance2)